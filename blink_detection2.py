...
shutil.rmtree('blink2')
shutil.rmtree('no_blink2')
os.mkdir('blink2')
os.mkdir('no_blink2')
...
for rect in faces1:
    ...
    if blink_ratio > BLINK_RATIO_THRESHOLD:
        ...
        cv2.imwrite("blink2/%s.jpg" %count, face)
        cv2.rectangle(img, (x, y), (x + w, y + h), (0, 255, 0), 2)
    else:
        face = img[y:y+h, x:x+w]
        cv2.imwrite("no_blink2/%s.jpg" %count, face)
        cv2.rectangle(img, (x, y), (x + w, y + h), (255, 0, 0), 2)
    count += 1

cv2.imwrite("picture/test_result2.jpg", img )
